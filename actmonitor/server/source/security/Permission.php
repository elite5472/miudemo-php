...
class Permission implements IPermission
{
	...
	private static function fillCache()
	{
		if(self::$cache === null)
		{
			...
			foreach($db->getAll('get.permission.all') as $result_set)
			{
	...
	public static function create($refid, $name = null, $description = null)
	{
		...
		try
		{
			...
			if(isset(self::$cache[$refid]))
				throw new \miu\db\DatabaseException("Permission $refid already exists.");
			...
			//Create record.
			$db->runQuery('set.permission:refid,item_description_id', array(
				'refid'=>$refid,
				'item_description_id'=>$item_description['id']
			));

			//Finally, get the result_set from the created user class.
			$result = new Permission($db->getFirst('get.permission:refid', array(
					'refid'=>$refid
			)));
